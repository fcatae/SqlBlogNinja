...
let CreateFileName (linkbase: string) (link: string)  =
    link.Substring(linkbase.Length).Replace('/', '-').Replace('%', '-')

let SaveContent (title, link, content)=
    let filename = sprintf "output\\posts\\%s.html" (CreateFileName "https://blogs.msdn.microsoft.com/fcatae/" link)
    let header = sprintf "<a link='%s'>%s</a>" link title
    File.WriteAllLines( filename , [header; content]) 


let elementsWithContent = 
    "input\\sqlblogninja.wordpress.xml" 
    |> XDocument.Load
    |> GetDescendent "item"
    |> Seq.filter( IsPost )
